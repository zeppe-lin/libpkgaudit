...
for suite in unit integration mechanism property cli header contract
 do
...
if find "$root/tests" -maxdepth 1 -type f \
    \( -name '*.cpp' -o -name '*.hpp' -o -name '*.py' -o -name '*.sh' \) |
    grep . >/dev/null; then
  fail 'uncategorized test source remains at tests/ root'
fi

for source in \
  "$root"/tests/unit/*_test.cpp \
  "$root"/tests/integration/*_test.cpp \
  "$root"/tests/mechanism/*_test.cpp \
  "$root"/tests/property/*_test.cpp
 do
...
for source in "$root"/tests/contracts/*.sh
 do
  base=$(basename "$source")
  stem=${base#check_}
  stem=${stem%.sh}
  if ! grep -F "$base" "$meson" >/dev/null &&
      ! grep -F "'$stem'" "$meson" >/dev/null; then
