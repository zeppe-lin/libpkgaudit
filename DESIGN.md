...
`pkgchk(1)` is an optional reference and compatibility client.  It owns:

* command-line parsing;
* opening `libpkgstate` beneath the selected root;
* adapting one immutable snapshot into `pkgaudit::inventory`;
* constructing the POSIX backend;
* diagnostic wording and verbosity;
* warning policy for ownership relations; and
* exit-status selection.

The adapter is a private tool source file.  It is not a library API and does not
appear in pkg-config metadata.
